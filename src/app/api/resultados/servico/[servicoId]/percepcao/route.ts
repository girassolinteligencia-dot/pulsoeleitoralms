...
function leituraRapida(total: number, saldo: number) {
  ...
  if (saldo >= 25) return { titulo: 'Avaliação positiva predominante', descricao: 'Entre as vozes registradas, a aprovação supera a insatisfação com folga.', tom: 'positivo' };
  if (saldo <= -25) return { titulo: 'Sinais de insatisfação elevados', descricao: 'Entre as vozes registradas, a insatisfação aparece acima da aprovação.', tom: 'negativo' };
  return { titulo: 'Percepção dividida', descricao: 'A leitura coletiva aparece equilibrada, sem vantagem forte de aprovação ou insatisfação.', tom: 'neutro' };
}
...
export async function GET(
  ...
  try {
    ...
    const satisfacao = manifestacoes.filter((m) => m.expectativa_vitoria === true).length;
    ...
    console.error('Erro ao buscar percepção do serviço:', error);
