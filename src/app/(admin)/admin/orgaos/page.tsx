    ...
    <div className="flex flex-col gap-8 pb-32">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="w-full md:w-auto flex gap-3">
          ...
          <button
            type="button"
            onClick={() => { setShowNovo(true); setNovoErro(''); setNovoSucesso(''); setNovoOrgao(novoVazio()); }}
      ...
      <div className="hidden md:block bg-surface-1 border border-border rounded-[2.5rem] overflow-hidden">
          ...
          <tbody>
            {orgaos.map(org => (
              <tr key={org.id} className={`border-b border-border/30 hover:bg-white/[0.01] transition-colors group ${org.status === 'Inativo' ? 'opacity-40' : ''}`}>
                <td className="px-8 py-5 text-xs font-bold text-text">{org.nome}</td>
                ...
                <td className="px-8 py-5 text-right">
                  <button type="button" onClick={() => setEditingOrgao(org)} className="text-[10px] font-bold text-primary hover:text-white uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all">
                    Editar
                  </button>
                </td>
              </tr>
        ...
        {!loading && orgaos.length === 0 && (
          <div className="p-20 text-center text-text-muted uppercase tracking-widest text-[10px]">
            Nenhum órgão encontrado{search ? ` para "${search}"` : ''}
          </div>
        )}
      </div>
        ...
        {orgaos.map(org => (
          <div key={org.id} className="bg-surface-1 border border-border rounded-3xl p-6 flex flex-col gap-4">
            <div className="flex justify-between items-start">
              ...
              <span className={`text-[8px] uppercase font-bold tracking-widest px-2 py-1 rounded-full ${org.status === 'Ativo' ? 'bg-positive/10 text-positive' : 'bg-white/5 text-text-muted'}`}>{org.status}</span>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-y-auto">
          ...
          <div className="relative w-full max-w-2xl bg-surface-1 border border-border rounded-[2.5rem] p-8 md:p-10 shadow-2xl my-8">
            ...
            <p className="text-[9px] text-text-muted uppercase tracking-widest mb-8">
              Cadastre prefeituras, câmaras, tribunais, MP, defensorias e outros órgãos públicos do MS.
            </p>
            ...
            <form onSubmit={handleCriar} className="flex flex-col gap-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <select value={novoOrgao.tipo} onChange={e => setNovoOrgao({ ...novoOrgao, tipo: e.target.value })} className={inputClass} required>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input type="text" value={novoOrgao.cidade} onChange={e => setNovoOrgao({ ...novoOrgao, cidade: e.target.value })} className={inputClass} placeholder="Ex: Campo Grande" required />
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <select value={novoOrgao.campanha_id} onChange={e => setNovoOrgao({ ...novoOrgao, campanha_id: e.target.value })} className={inputClass}>
            ...
            <form onSubmit={handleUpdate} className="flex flex-col gap-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="flex flex-col gap-2 md:col-span-2">
                  ...
                  <input type="text" value={editingOrgao.nome || ''} onChange={e => setEditingOrgao({ ...editingOrgao, nome: e.target.value })} className={inputClass} required />
                </div>
                <div className="flex flex-col gap-2">
                  <label className={labelClass}>Tipo</label>
                  <select value={editingOrgao.tipo || ''} onChange={e => setEditingOrgao({ ...editingOrgao, tipo: e.target.value })} className={inputClass}>
                    {TIPOS_ORGAO.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                  </select>
                </div>
                <div className="flex flex-col gap-2">
                  <label className={labelClass}>Cidade</label>
                  <input type="text" value={editingOrgao.cidade || ''} onChange={e => setEditingOrgao({ ...editingOrgao, cidade: e.target.value })} className={inputClass} required />
                </div>
                <div className="flex flex-col gap-2">
                  <label className={labelClass}>Campanha</label>
                  <select value={editingOrgao.campanha_id || ''} onChange={e => setEditingOrgao({ ...editingOrgao, campanha_id: e.target.value || null })} className={inputClass}>
              ...
              <div className="flex gap-4 mt-4">
                ...
                <button type="button" onClick={() => setEditingOrgao(null)} className="px-8 py-4 bg-transparent border border-border text-text-muted rounded-full text-[10px] font-bold uppercase tracking-widest hover:text-white transition-all">
                  Cancelar
                </button>
              </div>
            </form>
